...
ggplot()+
  geom_function(fun=dnorm, color="#8B2500", args = list(mean = 150, sd = 15), xlim=c(100,  200))+
  theme_bw()
...
ggplot(tabla_medias)+
  geom_histogram(aes(Media, y=after_stat(density)), fill="#27408B", color="black")+
  geom_function(fun=dnorm, color="#CD4F39", args = list(mean = 150, sd = 1.5), xlim=c(145,  155))+
  theme_bw()
...
ggplot(tabla_resumen)+
  ...
  geom_hline(aes(yintercept = media), color="#8B2252", linetype=2)+
   theme_bw()
...
ggplot(tabla_resumen)+
  ...
  geom_hline(aes(yintercept = media), color="#00868B", linetype=2)+
   theme_bw()
```
...
ggplot(tabla_poisson)+
  geom_segment(aes(x=x, xend=x, y=0, yend=prob), color="yellow")+
  geom_point(aes(x, prob), color="#FF6347", size=2)+
  theme_bw()
